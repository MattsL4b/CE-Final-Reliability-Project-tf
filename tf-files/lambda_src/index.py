...
def trigger_asynchronous_revalidation(
    cache_key: str,
    path: str,
    query_params: dict,
    ...
    try:
        ...
        logger.error(
            f"FAILED TO ENQUEUE ASYNC REVALIDATION key={cache_key} err={str(e)}"
        )
...
def acquire_lock(cache_key: str) -> bool:
    ...
    try:
        cache_table.put_item(
            Item={"cache_key": lock_key, "expires_at": ttl},
...
def lambda_handler(event, context):
    ...
    if method == "GET":
        cache_key, resource_base_path = generate_cache_key(
            authorization, path, query_params
        )
        now = int(time.time())
        cached_item = get_from_cache(cache_key)
        ...
        if cached_item and now < int(cached_item.get("soft_ttl", 0)):
            ...
            return build_response(
                status_code=200, body=cached_item["data"], cache_status="HIT"
            )

        # 2. Stale Cache Hit - Return instantly & trigger background revalidation
        if cached_item and "data" in cached_item:
            logger.info(
                f"CACHE STALE - Serving stale data and dispatching async revalidation key={cache_key}"
            )
            trigger_asynchronous_revalidation(
                cache_key, path, query_params, incoming_headers, authorization
            )
            return build_response(
                status_code=200,
        ...
        if not got_lock:
            ...
            if waited_item and "data" in waited_item:
                ...
                return build_response(
                    status_code=200, body=waited_item["data"], cache_status="HIT"
                )
        try:
            ...
            if upstream_status == 200:
                ...
                return build_response(
                    status_code=200, body=upstream_body, cache_status="MISS"
                )
    ...
    return build_response(
        status_code=status_code, body=response_body, cache_status="BYPASS"
    )
    ...
    for record in event["Records"]:
        try:
            ...
            if upstream_status == 200:
                ...
                logger.warning(
                    f"BACKGROUND REVALIDATION FAILED status={upstream_status} key={cache_key}"
                )
...
def generate_cache_key(
    authorization: str, path: str, query_params: dict
) -> tuple[str, str]:
    """
    ...
    filtered_params = {
        k: v for k, v in query_params.items() if k.lower() not in ignored_params
    }

    # Sort query string for deterministic keys
    query_string = (
        urllib.parse.urlencode(sorted(filtered_params.items()))
        if filtered_params
        else ""
    )
    ...
    try:
        cache_table.put_item(
            Item={
                ...
                "expires_at": now
                + hard_ttl_sec,  # DynamoDB native TTL attribute name (ttl key in main.tf)
            }
        )
        logger.info(
            f"CACHE STORED key={key} soft_ttl={soft_ttl_sec}s hard_ttl={hard_ttl_sec}s"
        )
    except Exception as error:
...
def fetch_from_hosp(
    method, path, query_params, incoming_headers, authorization, body=None, context=None
):
    url = f"{HOSP_BASE_URL}{path}"
    if query_params:
        url += f"?{urllib.parse.urlencode(query_params)}"

    outgoing_headers = {"Authorization": authorization, "Accept": "application/json"}

    content_type = incoming_headers.get("content-type") or incoming_headers.get(
        "Content-Type"
    )
    if content_type:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        request = urllib.request.Request(
            url=url, data=request_body, headers=outgoing_headers, method=method
        )
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=timeout) as response:
                ...
                logger.info(
                    f"HOSP RESPONSE method={method} path={path} status={response.status} attempt={attempt}"
                )
            ...
            logger.warning(
                f"HOSP HTTP ERROR method={method} path={path} status={status_code} attempt={attempt}"
            )
            ...
            if isinstance(error.reason, socket.timeout):
                status_code = 504
                logger.error(
                    f"HOSP TIMEOUT (URLError) method={method} path={path} attempt={attempt}"
                )
                if method == "GET" and attempts_remaining:
                    time.sleep(RETRY_DELAY_SECONDS)
                    continue
            ...
            status_code = 502
            logger.error(
                f"HOSP CONNECTION ERROR method={method} path={path} attempt={attempt} err={error}"
            )
            if method == "GET" and attempts_remaining:
                time.sleep(RETRY_DELAY_SECONDS)
                continue
...
def build_response(status_code, body, cache_status):
    ...
    return {
        ...
        "headers": {"Content-Type": "application/json", "X-Proxy-Cache": cache_status},
        "body": body,
    }
